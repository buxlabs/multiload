...
test("should be possible to load modules from different folders recursively (example 3)", () => {
  const modules = multiload(path.join(__dirname, "/../fixture/dir_25"), {
    recursive: true,
  });

  assert.ok(modules["one/index"]);
  assert.ok(modules["two/index"]);
  assert.ok(modules["three/lorem"]);
  assert.ok(modules["three/ipsum"]);

  const array = multiload(path.join(__dirname, "/../fixture/dir_25"), {
    recursive: true,
    format: "array",
  });

  assert.ok(array.includes(modules["one/index"]));
  assert.ok(array.includes(modules["two/index"]));
  assert.ok(array.includes(modules["three/lorem"]));
  assert.ok(array.includes(modules["three/ipsum"]));
});
